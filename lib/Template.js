...
const __toggleCollapseExpandButton = (button, ticketID) => {
  let directChildren = document.querySelectorAll(`div.ticket#T${ticketID} > *`)
  let childrenToNotHide = document.querySelectorAll(`div#T${ticketID} > button.toggle_ticket, h2.remarkup-header:first-of-type, h3.remarkup-header:first-of-type, div.ticket_users`)
  let childrenToToggle = [...Array.from(directChildren)].filter(e => !Array.from(childrenToNotHide).includes(e))
