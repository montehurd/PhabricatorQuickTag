...
const __firstOnscreenTicket = () => {
  ...
  return Array.from(document.querySelectorAll('div.ticket')).find((e, i, a) => {
    ...
    return (rect.top > insetTop + 1 || rect.top + rect.height > insetTop + 1)
  })
}
...
const __selectAndScrollToNextOrPreviousOnscreenTicket = event => {
  if (!__isShiftUpOrDownArrowEvent(event)) {
    return
  }
...
const __isShiftUpOrDownArrowEvent = event => {
  return (event.shiftKey == true && (event.code == 'ArrowDown' || event.code == 'ArrowUp'))
}
...
document.addEventListener('keyup', __selectAndScrollToNextOrPreviousOnscreenTicket)
